...
class Monitoring(Base):
    ...
    # de escala pois há algumas "memecoins" que chegam a 18 casas decimais após a vírgula (por isso o aumento no total
    # de dígitos também. Os demais que usam 2 é devido ao fato que as casas decimais não são tão relevantes.

    id = Column(Integer, primary_key=True)
    crypto_id = Column(Integer, ForeignKey('cryptos.id'))
    price = Column(Numeric(40,20), nullable=False)
    rank = Column(Integer)
